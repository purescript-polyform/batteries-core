...
suite :: TestSuite
suite = do
  Test.Unit.suite "Test.Polyform.Batteries.UrlEncoded.Duals" $ do
    Test.Unit.suite "Error accumulation" $ do
      Test.Unit.test " object" $ do
        let
          -- | Let's create a smiple form layout (`Array [String]`)
          field name =
            liftValidatorDualWith (const $ [name]) (const $ [name]) (required name identity)
          ...
          expected = ["foo", "bar"]
        do
          let emptyPayload = Decoded $ Map.fromFoldable []
          Tuple _ given ← runReporter form emptyPayload
          Assert.equal given expected

        do
          let onlyFoo = Decoded $ Map.fromFoldable ["foo" /\ ["value"]]
          Tuple _ given ← runReporter form onlyFoo
          Assert.equal given expected

        do
          let both = Decoded $ Map.fromFoldable ["foo" /\ ["value"], "bar" /\ ["value"]]
